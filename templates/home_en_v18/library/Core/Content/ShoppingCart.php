...
class CoreContentShoppingCart
{
            ...
            if (!$this->_cart->automaticSelectedShipment) {
                ...
                if (!empty($this->_component->layoutName) && $this->_component->layoutName == $this->_cart->layout) {
                    if (VmConfig::get('oncheckout_opc', 0)) {
                        ...
                        $shipmentContent .= JHtml::_('link', JRoute::_('index.php?option=com_virtuemart&view=cart&task=edit_shipment', $this->_component->useXHTML, $this->_component->useSSL), $this->_component->select_shipment_text, 'class=""');
                    }
                } else {
            ...
            if (!$this->_cart->automaticSelectedPayment) {
                ...
                if (!empty($this->_component->layoutName) && $this->_component->layoutName == $this->_cart->layout) {
                    if (VmConfig::get('oncheckout_opc', 0)) {
                        ...
                        $paymentContent .= JHtml::_('link', JRoute::_('index.php?option=com_virtuemart&view=cart&task=editpayment', $this->_component->useXHTML, $this->_component->useSSL), $this->_component->select_payment_text, 'class=""');
                    }
                } else {
    ...
    public function getCartTotals()
    {
        ...
        return array(
            ...
            'checkoutBtnText' => vmText::_('COM_VIRTUEMART_CHECKOUT_TITLE'),
        );
    }

    public function beginCheckoutForm() {
        $action = JRoute::_ ('index.php?option=com_virtuemart&view=cart' . '', $this->_component->useXHTML, $this->_component->useSSL);
        echo <<<STARTFORM
...
ENDFORM;
    ...
    public function getProductItems()
    {
        ...
        foreach ($this->_cart->products as $pkey => $prow) {
            ...
            //$productItem['name'] = JHtml::link ($prow->url, $prow->product_name);
...
QUANTITY;
    ...
    public function includeStyles($dir)
    {
        $styles = $dir . '/default_styles.php';
        $stylesContent = '';
        if (file_exists($styles)) {
            ob_start();
            include_once $dir . '/default_styles.php';
