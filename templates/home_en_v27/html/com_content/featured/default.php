...
if (count($funcsInfo)) {
    ...
    for ($i = 0; $i < $all; $i++) {
        ...
        if ($funcInfo['count'] > count($customItems) && $i + 1 !== $all) {
            continue;
        }

        while ($funcInfo['count'] > count($customItems)) {
            $customItems[] = null;
        }
        ...
        for ($j = 0; $j < count($customItems); $j++) {
            $item = $customItems[$j];
            $article = $item ? $component->article('featured', $item, $item->params) : null;

            ${'title' . $j} = $article && strlen($article->title) ? $this->escape($article->title) : '';
            ${'titleLink' . $j} = $article && strlen($article->titleLink) ? $article->titleLink : '';
            ${'readmore' . $j} = $article && strlen($article->readmore) ? $article->readmore : '';
            ${'readmoreLink' . $j} = $article && strlen($article->readmoreLink) ? $article->readmoreLink : '';
            ${'shareLink' . $j} = $article && strlen($article->shareLink) ? $article->shareLink : '';
            ${'content' . $j} = $article ? $article->intro(funcBalanceTags($article->intro)) : '';
            ...
            ${'image' . $j} = $image;
            ${'tags' . $j} = $article && count($article->tags) > 0 ? implode('', $article->tags) : '';
